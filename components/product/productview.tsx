...
const mockProducts: Product[] = [
  {
    id: 1,
    name: "Leawal | High Quality Leather Wallet",
    category: "Wallets",
    price: 6050,
    inStock: true,
    colors: [
...
const ProductCard: React.FC<{ product: Product }> = ({ product }) => (
  <div className="bg-white rounded-2xl p-4 shadow-sm hover:shadow-lg transition-shadow duration-300 border border-gray-100/50">
    ...
    <p className="font-bold text-lg my-2 text-black">
...
export default function ProductListingPage({
  ...
  const filteredProducts = useMemo(() => {
    if (showBestSellers) {
      // Simple best-sellers heuristic: show the first 6 mock products
      return mockProducts.slice(0, 6);
    }

    const cat = category ? category.toLowerCase() : "";
    return mockProducts.filter((product) => {
      if (cat && !product.category.toLowerCase().includes(cat)) return false;
      if (inStockOnly && !product.inStock) return false;
      if (product.price > maxPrice) return false;
      if (
        selectedColors.length > 0 &&
        !product.colors.some((pc) => selectedColors.includes(pc.name))
      )
        return false;
      return true;
    });
  }, [category, inStockOnly, maxPrice, selectedColors, showBestSellers]);
